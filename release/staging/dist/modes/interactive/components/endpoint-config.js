 ...
 */
import { Container, getKeybindings, Input, Spacer, Text } from "@hamr/tui";
...
export class EndpointConfigComponent extends Container {
    constructor(tui, onSave, onCancel, initialConfig) {
        super();
        // Form state
        this.presetIndex = 0;
        this.mode = "navigate";
    ...
    renderForm() {
        ...
        let idx = 0;
        const row = (label, value, _isField) => {
        ...
        if (this.mode === "navigate") {
            this.hintText.setText(rawKeyHint("↑↓", "navigate") +
                "  " +
    ...
    getSelectedFieldType() {
        ...
        if (this.selectedField < headerStart + headerCount)
            return "header";
        if (this.selectedField === headerStart + headerCount)
            return "add_header";
        if (this.selectedField === headerStart + headerCount + 1)
            return "save";
        return "cancel";
    }
    startEdit() {
    ...
    showHeaderEditView() {
        ...
        const hrow = (label, value, _field, selected) => {
            const prefix = selected ? theme.fg("accent", "→ ") : "  ";
            const lbl = theme.fg(selected ? "accent" : "text", `${label}: `);
            const val = theme.fg(selected ? "accent" : "dim", value);
            this.listContainer.addChild(new Text(`${prefix}${lbl}${val}`, 1, 0));
    ...
    navigateHeaderEdit(direction) {
        const fields = this.headerEdit?.isNew
    ...
    activateHeaderEdit() {
        ...
        switch (this.headerEditField) {
            ...
            case "secret":
                ...
                return;
            case "delete": {
                const idx = this.config.headers.findIndex((h) => h.key === this.headerEdit.key && h.value === this.headerEdit.value);
                if (idx >= 0) {
