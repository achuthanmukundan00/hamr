...
import { modelsAreEqual } from "@hamr/ai";
...
import { getHamrDefaultModel, loadHamrStartupConfig } from "./hamr/startup-config.js";
...
function findSessionFileByExactId(dir, sessionId) {
    ...
    for (const entry of entries) {
        ...
        if (stats.isDirectory()) {
            const found = findSessionFileByExactId(path, sessionId);
            if (found)
                return found;
            continue;
        }
...
async function createSessionManager(parsed, cwd, sessionDir, settingsManager) {
    ...
    if (parsed.fork) {
        if (parsed.sessionId) {
            const existingTarget = findSessionFileByExactId(sessionDir, parsed.sessionId) ??
                (await findLocalSessionByExactId(parsed.sessionId, cwd, sessionDir))?.path;
            if (existingTarget) {
...
export async function main(args, options) {
    resetTimings();
    const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.HAMR_OFFLINE) || isTruthyEnvFlag(process.env.PI_OFFLINE);
    if (offlineMode) {
    ...
    const startupBenchmark = isTruthyEnvFlag(process.env.HAMR_STARTUP_BENCHMARK) || isTruthyEnvFlag(process.env.PI_STARTUP_BENCHMARK);
