...
export class HandoffManager {
    ...
    createHandoff(sessionId, memory, options) {
        ...
        const filesChanged = [...new Set([...(options.filesChanged ?? []), ...(memoryManifest?.filesTouched ?? [])])].slice(0, 30);
...
const managersBySession = new Map();
function getManager(sessionId) {
...
// ── Tool registration ────────────────────────────────────────────────────────
export function registerHandoffTool(pi) {
    pi.registerTool(defineTool({
        name: "create_handoff",
        label: "Create handoff",
        description: "Create a structured handoff manifest from FTS5 memory. Consumable by subagents for context inheritance. Tracks depth (max 3).",
        ...
        parameters: Type.Object({
            ...
            reason: Type.Optional(Type.Union([Type.Literal("context_exhaustion"), Type.Literal("task_delegation"), Type.Literal("explicit")], {
