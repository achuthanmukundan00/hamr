 ...
 */
function identityKey(toolName, input) {
    switch (toolName) {
        case "read": {
...
// ─── Extension ──────────────────────────────────────────────────────────────
export const hamrReadLoopGuardExtension = (pi) => {
        ...
        if (history.length >= LOOP_THRESHOLD) {
            ...
            if (allSame) {
                ...
                if (now - lastNudge > COOLDOWN_MS) {
                    lastNudge = now;
                    if (!ctx.isIdle()) {
                        pi.sendUserMessage(`(You've called "${event.toolName}" with identical arguments ${LOOP_THRESHOLD} times in a row. ` +
                            `This looks like a loop — try a different approach.)`, { deliverAs: "steer" });
                    }
                }
            }
        }
...
//# sourceMappingURL=read-loop-guard.js.map