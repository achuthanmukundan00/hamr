 ...
 */
export function sanitizeReasoningTags(content) {
 ...
 */
export function fastJsonParse(raw) {
 ...
 */
export function generateCallId(provided, index) {
...
export function makeCall(name, args, opts = {}) {
 ...
 */
export function parsePythonicArgs(argsStr) {
    const result = {};
    if (!argsStr.trim())
        return result;
...
function tokenizePythonicArgs(input) {
    const tokens = [];
    let i = 0;
    while (i < input.length) {
        ...
        if (/[a-zA-Z_]/.test(ch)) {
            ...
            if (val === "True")
                tokens.push({ type: "name", value: "True" });
            else if (val === "False")
                tokens.push({ type: "name", value: "False" });
            else if (val === "None")
                tokens.push({ type: "name", value: "None" });
            else
...
function coercePythonicValue(token) {
    switch (token.type) {
        case "string":
            return token.value;
        case "number":
...
export function extractDelimitedBlocks(content, openTag, closeTag) {
    const blocks = [];
    const between = [];
    let after = "";
 ...
 */
export function findPattern(content, regex) {
