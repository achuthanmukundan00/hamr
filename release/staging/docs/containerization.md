...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.hamr/agent/extensions/gondolin
cd ~/.hamr/agent/extensions/gondolin
...
RUN npm install -g --ignore-scripts @skaft/hamr
...
Use a named volume for `/root/.hamr/agent` if you want container-local settings and sessions. Mounting your host `~/.hamr/agent` exposes host auth and session files to the container.
...
```bash
openshell sandbox upload hamr-sandbox ./repo /workspace
openshell sandbox download hamr-sandbox /workspace/repo ./repo-out
```

OpenShell providers can keep raw model API keys outside the sandbox.
When inference routing is configured, code inside the sandbox can call `https://inference.local`, and the gateway injects the configured provider credentials upstream.
Configure Hamr to use the corresponding OpenAI-compatible or Anthropic-compatible endpoint if you want model traffic to use this route.
