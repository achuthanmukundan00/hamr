> hamr can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to hamr's agent capabilities. Use it to embed hamr in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@skaft/hamr";
...
```typescript
import { createAgentSession, SessionManager } from "@skaft/hamr";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `hamr.sendMessage()`.
...
The `Agent` class (from `@hamr/agent`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.hamr/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.hamr/extensions/`)
- Project skills:
  - `.hamr/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.hamr/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.hamr/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@skaft/hamr";
...
// Default: uses ~/.hamr/agent/auth.json and ~/.hamr/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@skaft/hamr";
...
The `edit` tool returns `details.diff` for Hamr's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@skaft/hamr";
...
import { createAgentSession, defineTool } from "@skaft/hamr";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `hamr.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `hamr.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.hamr/agent/extensions/`, `.hamr/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `hamr.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@skaft/hamr";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@skaft/hamr";
...
1. Global: `~/.hamr/agent/settings.json`
2. Project: `<cwd>/.hamr/settings.json`
