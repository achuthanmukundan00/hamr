> hamr can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@hamr/tui`](https://github.com/skaft-software/hamr/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@hamr/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `HAMR_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@hamr/tui";
...
**In custom tools** via `hamr.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = hamr.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@hamr/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@hamr/tui";
```
...
```typescript
import { matchesKey, Key } from "@hamr/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@hamr/tui";
...
```typescript
import { getMarkdownTheme } from "@skaft/hamr";
...
Set `HAMR_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout (`PI_TUI_WRITE_LOG` also works).

```bash
HAMR_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@hamr/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@skaft/hamr";
import { DynamicBorder } from "@skaft/hamr";
import { Container, type SelectItem, SelectList, Text } from "@hamr/tui";
...
```typescript
import { BorderedLoader } from "@skaft/hamr";

hamr.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@hamr/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@skaft/hamr";
import { Container, type SettingItem, SettingsList, Text } from "@hamr/tui";

hamr.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while hamr is streaming a response.
...
// Restore hamr's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@hamr/tui";
