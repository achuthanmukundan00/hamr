...
Extensions are TypeScript modules that extend hamr's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.
...
**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `hamr.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `hamr.registerCommand()`
- **Session persistence** - Store state that survives restarts via `hamr.appendEntry()`
...
```typescript
import type { ExtensionAPI } from "@skaft/hamr";
import { Type } from "typebox";

export default function (hamr: ExtensionAPI) {
  // React to events
  hamr.on("session_start", async (_event, ctx) => {
    ctx.ui.notify("Extension loaded!", "info");
  });

  hamr.on("tool_call", async (event, ctx) => {
...
| Package | Purpose |
|---------|---------|
| `@skaft/hamr` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
...
export default function (hamr: ExtensionAPI) {
  // Subscribe to events
  hamr.on("event_name", async (event, ctx) => {
  ...
  hamr.registerTool({ ... });
  hamr.registerCommand("name", { ... });
  hamr.registerShortcut("ctrl+x", { ... });
  hamr.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, hamr awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `hamr.registerProvider()` are flushed.
...
export default async function (hamr: ExtensionAPI) {
  ...
  hamr.registerProvider("local-openai", {
...
thinking level changes (settings, keybinding, hamr.setThinkingLevel())
...
```typescript
hamr.on("project_trust", async (event, ctx) => {
...
```typescript
hamr.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
hamr.on("session_start", async (event, ctx) => {
...
```typescript
hamr.on("session_before_switch", async (event, ctx) => {
...
```typescript
hamr.on("session_before_fork", async (event, ctx) => {
...
```typescript
hamr.on("session_before_compact", async (event, ctx) => {
...
hamr.on("session_compact", async (event, ctx) => {
...
```typescript
hamr.on("session_before_tree", async (event, ctx) => {
...
hamr.on("session_tree", async (event, ctx) => {
...
```typescript
hamr.on("session_shutdown", async (event, ctx) => {
...
```typescript
hamr.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Hamr uses to build the system prompt. This lets you inspect what Hamr has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
```typescript
hamr.on("agent_start", async (_event, ctx) => {});

hamr.on("agent_end", async (event, ctx) => {
...
```typescript
hamr.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

hamr.on("turn_end", async (event, ctx) => {
...
```typescript
hamr.on("message_start", async (event, ctx) => {
  // event.message
});

hamr.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

hamr.on("message_end", async (event, ctx) => {
...
```typescript
hamr.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

hamr.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

hamr.on("tool_execution_end", async (event, ctx) => {
...
```typescript
hamr.on("context", async (event, ctx) => {
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Hamr's system prompt string rather than the final serialized provider payload.

```typescript
hamr.on("before_provider_request", (event, ctx) => {
...
```typescript
hamr.on("after_provider_response", (event, ctx) => {
...
```typescript
hamr.on("model_select", async (event, ctx) => {
...
```typescript
hamr.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `hamr.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
```typescript
import { isToolCallEventType } from "@skaft/hamr";
...
```typescript
import { isBashToolResult } from "@skaft/hamr";

hamr.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@skaft/hamr";

hamr.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap hamr's built-in local bash backend
...
Request a graceful shutdown of hamr.
...
Returns Hamr's current system prompt string.
...
```typescript
hamr.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Hamr currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `hamr` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@skaft/hamr";

hamr.registerCommand("switch", {
...
- Captured old `hamr` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
hamr.registerCommand("reload-runtime", {
...
export default function (hamr: ExtensionAPI) {
  ...
  hamr.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      hamr.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### hamr.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### hamr.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`hamr.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `hamr.getAllTools()` and are callable by the LLM without `/reload`.

Use `hamr.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### hamr.sendMessage(message, options?)
...
### hamr.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
hamr.sendUserMessage("What is 2+2?");

// With content array (text + images)
hamr.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
hamr.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
hamr.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### hamr.appendEntry(customType, data?)

Persist extension state (does NOT participate in LLM context).

```typescript
hamr.appendEntry("my-state", { count: 42 });
...
### hamr.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
hamr.setSessionName("Refactor auth module");
```
...
```typescript
const name = hamr.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### hamr.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
hamr.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
hamr.setLabel(entryId, undefined);
...
### hamr.registerCommand(name, options)
...
```typescript
const commands = hamr.getCommands();
...
### hamr.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### hamr.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
hamr.registerShortcut("ctrl+shift+p", {
...
### hamr.registerFlag(name, options)

Register a CLI flag.

```typescript
hamr.registerFlag("plan", {
...
// Check value
if (hamr.getFlag("plan")) {
  // Plan mode enabled
}
```

### hamr.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await hamr.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### hamr.getActiveTools() / hamr.getAllTools() / hamr.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `hamr.getActiveTools()` returns the active tool names as `string[]`; `hamr.getAllTools()` returns metadata for all configured tools.

```typescript
const active = hamr.getActiveTools(); // ["read", "bash", ...]
const all = hamr.getAllTools();
// all = [{
//   name: "read",
...
hamr.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
hamr.setActiveTools(["read", "bash"]); // Switch to read-only
```

`hamr.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await hamr.setModel(model);
  if (!success) {
...
### hamr.getThinkingLevel() / hamr.setThinkingLevel(level)
...
```typescript
const current = hamr.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
hamr.setThinkingLevel("high");
```
...
```typescript
hamr.events.on("my:event", (data) => { ... });
hamr.events.emit("my:event", { ... });
```

### hamr.registerProvider(name, config)
...
hamr.registerProvider("my-proxy", {
  name: "My Proxy",
...
hamr.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
hamr.registerProvider("corporate-ai", {
...
```typescript
hamr.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    hamr.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `hamr.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `hamr.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@skaft/hamr";
...
hamr.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via hamr.exec (captured from extension closure)
    const result = await hamr.exec("some-command", [], { signal });
...
- [read.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@skaft/hamr";
...
For `user_bash`, extensions can reuse hamr's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The bash tool also supports a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@skaft/hamr";
...
export default function (hamr: ExtensionAPI) {
  ...
  hamr.registerTool({ name: "db_connect", ... });
  hamr.registerTool({ name: "db_query", ... });
  hamr.registerTool({ name: "db_close", ... });
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/skaft-software/hamr/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@skaft/hamr";
...
```typescript
hamr.on("session_start", (_event, ctx) => {
  ctx.ui.addAutocompleteProvider((current) => ({
    triggerCharacters: ["#"],
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@skaft/hamr";
...
hamr.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `hamr.sendMessage()`:
...
```typescript
import { highlightCode, getLanguageFromPath } from "@skaft/hamr";
...
| `event-bus.ts` | Inter-extension events | `hamr.events` |
| **Session Metadata** |||
