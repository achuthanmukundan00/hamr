...
Hamr loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.hamr/agent/themes/*.json`
- Project: `.hamr/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `hamr.themes` entries in `package.json`
...
```bash
mkdir -p ~/.hamr/agent/themes
vim ~/.hamr/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/skaft-software/hamr/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
