#!/usr/bin/env bash
# auto-research-loop.sh — Self-improvement loop for Hamr.
#
# Repeatedly:
...
#      and applies exactly one code patch to Hamr.
...
HAMR_CMD=""
DRY_RUN=false
FIXTURE="${HAMR_BENCH_FIXTURE:-validate-email}"
PATIENCE=1
...
BENCH_SCRIPT="$REPO_ROOT/scripts/run-hamr-benchmark.sh"
IMPROVEMENT_PROMPT="$REPO_ROOT/benchmarks/hamr-auto-research/improvement-agent-prompt.md"
...
if [ ! -f "$REPO_ROOT/dist/cli.js" ]; then
  echo "[loop] Building Hamr..."
  (cd "$REPO_ROOT" && bun run build) || { echo "[loop] Build failed"; exit 1; }
fi
[ -z "$HAMR_CMD" ] && HAMR_CMD="bun $REPO_ROOT/dist/cli.js"

# ─── Forbidden paths ────────────────────────────────────────
FORBIDDEN_PATHS=(
  "benchmark-artifacts/"
  "benchmarks/hamr-auto-research/fixtures/"
  "benchmarks/hamr-auto-research/README.md"
  "benchmarks/hamr-auto-research/improvement-agent-prompt.md"
  "scripts/run-hamr-benchmark.sh"
  "scripts/score-hamr-benchmark.mjs"
...
# Conditionally forbid bun.lock
cd "$REPO_ROOT"
if git diff --quiet -- bun.lock && git diff --cached --quiet -- bun.lock; then
  FORBIDDEN_PATHS+=("bun.lock")
else
  echo "[loop] Note: bun.lock already modified — excluded from forbidden list."
fi
...
run_benchmark() {
  ...
  bash "$BENCH_SCRIPT" "$run_id" "$LOOP_RUN_DIR" --timeout-seconds "$TIMEOUT_SECONDS" --hamr-cmd "$HAMR_CMD" --fixture "$FIXTURE" >&2
...
EOF
  ...
  # ── Step 2.6: Verify candidate patch (bun run verify) ───
  # Cheap deterministic gate: reject broken patches before
  # spending time on the expensive candidate benchmark.
  VERIFY_PASSED=null
  VERIFY_EXIT=null
  if [ "$DRY_RUN" = true ]; then
    echo "[loop] DRY RUN: skipping bun run verify."
    VERIFY_PASSED=true
    VERIFY_EXIT=0
  else
    echo "[loop] Running bun run verify..."
    VERIFY_OUT="$ITER_DIR/verify-output.txt"
    cd "$REPO_ROOT"
    VERIFY_EXIT=0
    bun run verify > "$VERIFY_OUT" 2>&1 || VERIFY_EXIT=$?
    if [ "$VERIFY_EXIT" -eq 0 ]; then
      echo "[loop] ✓ bun run verify passed."
      VERIFY_PASSED=true
    else
      echo "[loop] ✗ bun run verify FAILED (exit $VERIFY_EXIT)."
