...
# ─── Build agent prompt ───────────────────────────────────────
BASE_PROMPT_FILE="$REPO_ROOT/benchmarks/hamr-auto-research/improvement-agent-prompt.md"
...
if [ -f "$BASE_DIR/hamr-config-sanitized.toml" ]; then
  echo "- Sanitized config: \`${BASE_DIR}/hamr-config-sanitized.toml\`" >> "$AGENT_PROMPT_FILE"
fi
