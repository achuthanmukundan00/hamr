...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "hamr-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("HAMR_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.HAMR_CODING_AGENT_DIR!, "themes", "foo.json");
