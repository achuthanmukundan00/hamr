...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@hamr/coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @hamr/coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@hamr/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@hamr/coding-agent")).toBe(
			"Update @hamr/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@hamr/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@hamr/coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @hamr/coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@hamr/coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @hamr/coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@hamr/coding-agent";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@hamr/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@hamr/coding-agent")).toContain("the install path is not writable");
	});
});
