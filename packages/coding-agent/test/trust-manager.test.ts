...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".hamr", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".hamr", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".hamr", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".hamr"), { recursive: true });
			writeFileSync(join(cwd, ".hamr", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".hamr"), { recursive: true, force: true });
