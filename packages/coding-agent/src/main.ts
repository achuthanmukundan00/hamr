...
import { type ImageContent, modelsAreEqual } from "@hamr/ai";
...
import { getHamrDefaultModel, loadHamrStartupConfig } from "./hamr/startup-config.ts";
...
function findSessionFileByExactId(dir: string | undefined, sessionId: string): string | undefined {
	...
	for (const entry of entries) {
		...
		if (!stats.isFile() || !path.endsWith(".jsonl")) continue;
		try {
	...
	if (parsed.fork) {
		if (parsed.sessionId) {
			const existingTarget =
				findSessionFileByExactId(sessionDir, parsed.sessionId) ??
