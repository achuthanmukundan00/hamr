...
function taskLooksLikeBashCommand(task: string): boolean {
	...
	// If the first line starts with a sentence-cased word and is
	// longer than 8 words (≈ natural-language instruction), decline.
	...
	const shellStarters = new Set([
		"bash", "sh", "zsh", "curl", "wget", "cat", "ls", "grep", "find",
		"sed", "awk", "python3", "python", "node", "npm", "npx", "yarn",
		"pnpm", "cargo", "go", "rustc", "make", "cmake", "docker", "podman",
		"git", "ssh", "scp", "rsync", "tar", "unzip", "mkdir", "rm", "cp",
		"mv", "chmod", "chown", "sudo", "brew", "apt", "dnf", "systemctl",
		"launchctl", "kill", "killall", "ps", "top", "df", "du", "mount",
		"export", "source", ".", "echo", "printf", "env", "which", "type",
		"test", "[", "true", "false", "exit", "exec", "xargs", "tee",
		"head", "tail", "sort", "uniq", "wc", "cut", "tr", "jq",
	]);
