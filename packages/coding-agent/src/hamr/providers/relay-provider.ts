...
function booleanField(entry: Record<string, unknown>, fields: string[]): boolean | undefined {
...
function extractSupportsVision(entry: Record<string, unknown>): boolean | undefined {
	...
	if (meta) {
		...
		if (metaExplicit !== undefined) return metaExplicit;
	}
...
export async function discoverRelayModels(
	...
	try {
		...
		const models: DiscoveredRelayModel[] = [];
		for (const entry of entries) {
			...
			const apiLevels = Array.isArray(entry.thinking_levels)
				? (entry.thinking_levels as unknown[]).map((l) => String(l)).filter((l) => KNOWN_THINKING_LEVELS.includes(l))
				: [];
