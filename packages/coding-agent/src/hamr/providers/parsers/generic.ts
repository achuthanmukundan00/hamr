...
import type { ParsedToolCall, ToolCallParseResult, ToolCallParser } from "./types.ts";
import { extractDelimitedBlocks, generateCallId, safeJsonParse, sanitizeReasoningTags } from "./utils.ts";
...
export const genericParser: ToolCallParser = {
	...
	parse(content: string): ToolCallParseResult {
		...
		let xmlNonToolContent: string | undefined;
		if (
			sawMalformedBlock &&
			calls.length === 0 &&
