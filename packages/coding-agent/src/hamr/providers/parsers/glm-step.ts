...
import type { ToolCallParseResult, ToolCallParser } from "./types.ts";
import { coerceValue, extractDelimitedBlocks, generateCallId, safeJsonParse, sanitizeReasoningTags } from "./utils.ts";
...
function parseSpecialTokenBlocks(content: string, token: string, parserId: string) {
	const calls: Array<import("./types.ts").ParsedToolCall> = [];
