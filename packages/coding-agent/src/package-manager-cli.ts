...
import { Markdown, type MarkdownTheme } from "@hamr/tui";
...
import { getLatestHamrRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|hamr] [--self] [--extensions] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Options:
  -l, --local       Install project-locally (.hamr/settings.json)
...
Options:
  -l, --local       Remove from project settings (.hamr/settings.json)
  -a, --approve     Trust project-local files for this command
  -na, --no-approve Ignore project-local files for this command
...
Update hamr and installed packages.

Options:
  --self                  Update hamr only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall hamr even if the current version is latest

Short forms:
  ${APP_NAME} update                Update hamr and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update hamr           Update hamr only (self and pi work as aliases)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "hamr" || source === "pi";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of hamr executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestHamrRelease(VERSION);
	...
	try {
		...
		child.on("close", (code) => {
			...
			if (code === 0 && newVersion && newVersion !== VERSION) {
				...
				console.log(
					chalk.yellow(
						`${APP_NAME} v${VERSION} is still running. Restart your terminal or run: npm install -g @skaft/hamr@latest`,
					),
				);
			}
		});
 ...
 */
async function rebuildBetterSqlite3(): Promise<void> {
	...
	let rebuild: { command: string; args: string[] } | null = null;
	...
	try {
		...
		console.warn(
			chalk.yellow(`Warning: Failed to rebuild better-sqlite3: ${err instanceof Error ? err.message : String(err)}`),
		);
