<p align="center">
  <a href="https://hamr.dev">
    <img alt="hamr logo" src="https://hamr.dev/logo-auto.svg" width="128">
  </a>
</p>
<p align="center">
  <a href="https://discord.com/invite/3cU7Bz4UPx"><img alt="Discord" src="https://img.shields.io/badge/discord-community-5865F2?style=flat-square&logo=discord&logoColor=white" /></a>
  <a href="https://www.npmjs.com/package/@hamr/coding-agent"><img alt="npm" src="https://img.shields.io/npm/v/@hamr/coding-agent?style=flat-square" /></a>
</p>
<p align="center">
  <a href="https://hamr.dev">hamr.dev</a> domain graciously donated by
  <br /><br />
...
If you use Hamr for open source work, please share your coding agent sessions.
...
`--ignore-scripts` disables dependency lifecycle scripts during install. Hamr does not require install scripts for normal npm installs.
...
Then just talk to Hamr. By default, Hamr gives the model four tools: `read`, `write`, `edit`, and `bash`. The model uses these to fulfill your requests. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [pi packages](#hamr-packages).
...
For each built-in provider, Hamr maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
**Custom providers & models:** Add providers via `~/.hamr/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.hamr/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Hamr can receive the follow-up shortcut.
...
| Location | Scope |
|----------|-------|
| `~/.hamr/agent/settings.json` | Global (all projects) |
| `.hamr/settings.json` | Project (overrides global) |

See [docs/settings.md](docs/settings.md) for all options.

### Project Trust

On interactive startup, Hamr asks before trusting a project folder that contains project-local settings, resources, or project `.agents/skills` and has no saved decision for the folder or a parent folder in `~/.hamr/agent/trust.json`. Trusting a project allows Hamr to load `.hamr/settings.json` and `.hamr` resources, install missing project packages, and execute project extensions.

Before the trust decision, Hamr loads only context files, user/global extensions, and CLI `-e` extensions so they can handle the `project_trust` event. Project-local extensions, project package-managed extensions, and project settings are loaded only after the project is trusted. This split also applies when switching to a session from a different cwd whose trust has not been resolved in the current process.
...
Hamr loads .AGENTS.md` (or `CLAUDE.md`) at startup from:
- `~/.hamr/agent/AGENTS.md` (global)
...
Replace the default system prompt with `.hamr/SYSTEM.md` (project) or `~/.hamr/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.hamr/agent/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.hamr/agent/prompts/`, `.hamr/prompts/`, or a [pi package](#hamr-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).
...
```markdown
<!-- ~/.hamr/agent/skills/my-skill/SKILL.md -->
# My Skill
...
Place in `~/.hamr/agent/skills/`, `~/.agents/skills/`, `.hamr/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [pi package](#hamr-packages) to share with others. See [docs/skills.md](docs/skills.md).

### Extensions

<p align="center"><img src="docs/images/doom-extension.png" alt="Doom Extension" width="600"></p>

TypeScript modules that extend Hamr with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. Hamr waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make Hamr look like Claude Code
- Games while waiting (yes, Doom runs)
- ...anything you can dream up

Place in `~/.hamr/agent/extensions/`, `.hamr/extensions/`, or a [pi package](#hamr-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and Hamr immediately applies changes.

Place in `~/.hamr/agent/themes/`, `.hamr/themes/`, or a [pi package](#hamr-packages) to share with others. See [docs/themes.md](docs/themes.md).

### Hamr Packages

Bundle and share extensions, skills, prompts, and themes via npm or git. Find packages on [npmjs.com](https://www.npmjs.com/search?q=keywords%3Ahamr-package) or [Discord](https://discord.com/channels/1456806362351669492/1457744485428629628).
...
Hamr is aggressively extensible so it doesn't have to dictate your workflow. Features that other tools bake in can be built with [extensions](#extensions), [skills](#skills), or installed from third-party [pi packages](#hamr-packages). This keeps the core minimal while letting you shape Hamr to fit how you work.

**No MCP.** Build CLI tools with READMEs (see [Skills](#skills)), or build an extension that adds MCP support. [Why?](https://mariozechner.at/posts/2025-11-02-what-if-you-dont-need-mcp/)

**No sub-agents.** There's many ways to do this. Spawn Hamr instances via tmux, or build your own with [extensions](#extensions), or install a package that does it your way.
...
|----------|-------------|
| `PI_CODING_AGENT_DIR` | Override config directory (default: `~/.hamr/agent`) |
...
- [@hamr/ai](https://www.npmjs.com/package/@hamr/ai): Core LLM toolkit
- [@hamr/agent](https://www.npmjs.com/package/@hamr/agent): Agent framework
- [@hamr/tui](https://www.npmjs.com/package/@hamr/tui): Terminal UI components
