...
### Low-hanging fruit
- **Help text rebrand:** `package-manager-cli.ts` still says "Update pi" / "pi update" — should say "hamr update"
...
- **GitHub URLs in migrations.ts:** Still point to `earendil-works/pi-mono` repo

### Medium effort
- **Version string:** Currently `0.79.6` (inherited from Pi). Consider bumping to a Hamr-specific version (e.g. `1.0.0`).
- **Pi→Hamr in CHANGELOG.md:** Historical entries reference Pi packages. These are archival and can stay, but new entries should use `@hamr/*` naming.
- **npm-shrinkwrap.json:** References `pi-tui` in resolved URLs — auto-generated, updates on next `npm install`
- **P0 Checklists:** Write comprehensive Demo Gates + Brand Gate docs (user provided detailed checklist)

### Large effort (deferred)
- **Architecture simplification:** Toward `hamr theme + sexy-tui-rs + pi fork agent` — needs design doc
...
```
hamr monorepo
├── packages/tui/         → @hamr/tui     (fork of pi-tui, shared terminal UI components)
├── packages/ai/          → @hamr/ai      (model providers, API clients)
├── packages/agent/       → @hamr/agent   (agent loop, tools, sessions)
└── packages/coding-agent → @hamr/coding-agent  (CLI, TUI mode, extensions)
