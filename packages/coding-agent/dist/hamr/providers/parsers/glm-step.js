 ...
 */
import { coerceValue, extractDelimitedBlocks, generateCallId, safeJsonParse, sanitizeReasoningTags } from "./utils.js";
// ─── GLM 4.5 ──────────────────────────────────────────────
export const glm45Parser = {
    id: "glm45",
...
// ─── Shared helpers ───────────────────────────────────────
function parseSpecialTokenBlocks(content, token, parserId) {
    const calls = [];
...
function parseHermesFallback(sanitized, parserId) {
...
function parseQwenFallback(sanitized, parserId) {
    ...
    for (let i = 0; i < delimited.blocks.length; i++) {
        ...
        for (const param of block.matchAll(paramRegex)) {
            ...
            if (key)
                args[key] = coerceValue((param[2] ?? "").trim());
        }
        calls.push({
            id: generateCallId(undefined, i + 1),
            name: fnMatch[1].trim(),
...
// ─── Factories ────────────────────────────────────────────
export function createGlm45Parser() {
    return glm45Parser;
}
export function createGlm47Parser() {
    return glm47Parser;
}
export function createStep3Parser() {
    return step3Parser;
}
export function createStep3p5Parser() {
    return step3p5Parser;
}
//# sourceMappingURL=glm-step.js.map