 ...
 */
function extractJsonRegion(text) {
...
function fixInnerQuotes(json) {
    ...
    for (let i = 0; i < json.length; i++) {
        ...
        if (ch === '"') {
            if (!inString) {
                inString = true;
                inKey = !colonSeen; // after colon, we're in a value
                colonSeen = false;
                result.push(ch);
            }
            else {
 ...
 */
function fixTruncatedObject(json) {
