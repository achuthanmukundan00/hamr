...
function isInside(candidate, prefix) {
    if (candidate === prefix)
        return true;
...
export class PathGuard {
    ...
    isExplicitlyAllowed(candidate) {
        for (const a of this.allowed) {
            if (candidate === a || isInside(candidate, a))
                return true;
        }
        return false;
    }
     ...
     */
    deniedWriteReason(absolutePath) {
        if (!this.enabled)
            return undefined;
        const candidate = normalizeForCompare(absolutePath);
        if (this.isExplicitlyAllowed(candidate))
            return undefined;
...
function isGitHooksDir(absolutePath) {
