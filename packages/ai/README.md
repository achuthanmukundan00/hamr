...
TypeBox exports are re-exported from `@hamr/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@hamr/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@hamr/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@hamr/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@hamr/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@hamr/ai';
...
```typescript
import { getModel, stream } from '@hamr/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@hamr/ai';
...
```typescript
import { Model, stream } from '@hamr/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@hamr/ai';
...
```typescript
import { getModel, complete, Context } from '@hamr/ai';
...
```typescript
import { Context, getModel, complete } from '@hamr/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@hamr/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@hamr/ai';
...
```bash
npx @hamr/ai login              # interactive provider selection
npx @hamr/ai login anthropic    # login to specific provider
npx @hamr/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@hamr/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@hamr/ai/oauth';
...
```typescript
import { getModel, complete } from '@hamr/ai';
import { getOAuthApiKey } from '@hamr/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@hamr/ai`
