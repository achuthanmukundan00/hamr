...
async function* mapCodexEvents(events) {
    for await (const event of events) {
        ...
        if (type === "error") {
            ...
            const code = event.code || errorDetail?.code || errorDetail?.type || "";
            const message = event.message || errorDetail?.message || "";
...
function buildBaseCodexHeaders(initHeaders, additionalHeaders, accountId, token) {
    ...
    headers.set("originator", "hamr");
    const userAgent = _os ? `hamr (${_os.platform()} ${_os.release()}; ${_os.arch()})` : "hamr (browser)";
