...
export class AgentHarness {
    ...
    async executeTurn(turnState, text, options) {
        ...
        const runResultPromise = (async () => {
            try {
                return await runAgentLoop(messages, this.createContext(turnState, beforeResult?.systemPrompt), this.createLoopConfig(getTurnState, setTurnState), (event) => this.handleAgentEvent(event, abortController.signal), abortController.signal, abortController.signal, this.createStreamFn(getTurnState));
            }
            catch (error) {
                try {
