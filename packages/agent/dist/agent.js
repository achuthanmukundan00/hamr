import { streamSimple, } from "@hamr/ai";
...
export class Agent {
    ...
    async runPromptMessages(messages, options = {}) {
        await this.runWithLifecycle(async (lifecycleSignal, toolSignal) => {
            await runAgentLoop(messages, this.createContextSnapshot(), this.createLoopConfig(options), (event) => this.processEvents(event), lifecycleSignal, toolSignal, this.streamFn);
        });
    }
    async runContinuation() {
        await this.runWithLifecycle(async (lifecycleSignal, toolSignal) => {
            await runAgentLoopContinue(this.createContextSnapshot(), this.createLoopConfig(), (event) => this.processEvents(event), lifecycleSignal, toolSignal, this.streamFn);
        });
    }
