 ...
 */
import { EventStream, streamSimple, validateToolArguments, } from "@hamr/ai";
/**
 * Start an agent loop with a new prompt message.
 * The prompt is added to the context and events are emitted for it.
 */
export function agentLoop(prompts, context, config, signal, toolSignal, streamFn) {
    const stream = createAgentStream();
    void runAgentLoop(prompts, context, config, async (event) => {
        stream.push(event);
    }, signal, toolSignal, streamFn).then((messages) => {
        stream.end(messages);
    });
    return stream;
}
 ...
 */
export function agentLoopContinue(context, config, signal, toolSignal, streamFn) {
...
export async function runAgentLoop(prompts, context, config, emit, signal, toolSignal, streamFn) {
...
export async function runAgentLoopContinue(context, config, emit, signal, toolSignal, streamFn) {
    if (context.messages.length === 0) {
        throw new Error("Cannot continue: no messages in context");
    }
    if (context.messages[context.messages.length - 1].role === "assistant") {
 ...
 */
async function runLoop(initialContext, newMessages, initialConfig, signal, toolSignal, emit, streamFn) {
 ...
 */
async function executeToolCalls(currentContext, assistantMessage, config, signal, toolSignal, emit) {
...
async function executeToolCallsSequential(currentContext, assistantMessage, toolCalls, config, signal, toolSignal, emit) {
...
async function executeToolCallsParallel(currentContext, assistantMessage, toolCalls, config, signal, toolSignal, emit) {
    const finalizedCalls = [];
    for (const toolCall of toolCalls) {
        await emit({
            type: "tool_execution_start",
...
async function executePreparedToolCall(prepared, toolSignal, emit) {
    const updateEvents = [];
    let acceptingUpdates = true;
    try {
        const result = await prepared.tool.execute(prepared.toolCall.id, prepared.args, toolSignal, (partialResult) => {
            if (!acceptingUpdates)
                return;
