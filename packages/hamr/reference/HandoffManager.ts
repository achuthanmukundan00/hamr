...
import { type AgentClient, type AgentTurnResult, Session } from '../session/Session';
import type { SpanTracer } from '../telemetry/SpanTracer';
import { createInspectionLedger } from '../tools';
import type { HandoffManagerOptions, HandoffManifest, HandoffReason, HandoffResult } from './types';

export type { HandoffManagerOptions, HandoffManifest, HandoffReason, HandoffResult };
...
export class HandoffManager {
  ...
  buildChildContext(manifest: HandoffManifest, skillMessages?: string[]): Array<{ role: string; content: string }> {
    ...
    const searchTerms = manifest.suggestedSearchTerms.slice(0, 8);
    if (searchTerms.length > 0) {
      contextLines.push(
        '### Search Terms for Memory',
        `Use \`search_memory\` with these terms to retrieve parent context: ${searchTerms.join(', ')}`,
        '',
      );
    }
  ...
  async executeHandoff(params: {
    ...
    // Build child session with system prompt + handoff manifest.
    // Must use createConversation() so the child gets the Hamr system prompt
    // (tool definitions, mode rules, behavioral guidance). Without it the child
    // model has no context for how to use tools or what behaviors are expected.
    const baseConversation = Session.createConversation({
      skillMessages: params.skillMessages,
      hasMutationTools: params.mode !== 'read-only',
    });
    const childConversation = {
      messages: [...baseConversation.messages, ...childContext],
      inspectionLedger: createInspectionLedger(),
      latestCompaction: null,
      tokenLedger: createTokenLedger(),
      assemblyStats: null,
  ...
  async tryHandoff(params: {
    ...
    const manifest = this.generateManifest({
      ...
      contextWindowUsed: params.contextWindowUsed,
      conversationMessages: params.parentSession.conversation.messages.slice(-10).map((m) => ({
        role: m.role,
