...
import type { HandoffManager } from '../handoff/HandoffManager';
import type { HandoffManifest } from '../handoff/types';
import type { Session } from '../session/Session';
import type { OrchestrationPlan, OrchestrationResult, SubAgentResult, SubTask } from '../session/types';
import { detectConflicts } from './conflict-detector';
import { DependencyCycleError, resolveTaskDependencies } from './dependency-resolver';
...
export class OrchestrationManager {
  ...
  static async execute(
    ...
    if (mode === 'parallel' && mutatingTasks.length > 0 && hasOverlappingFileScopes(subTasks)) {
      // Production safety: child sessions share one working tree. Scope hints
      // are advisory to the model, not a hard write barrier, so any mutating
      // parallelism with overlapping file scopes can race/corrupt edits.
      // Keep read-only recon and disjoint-scoped parallel writes safe, but
      // serialize when scopes overlap.
      mode = 'sequential';
    }
  ...
  private static async runSequentialMode(
    ...
    for (let g = 0; g < groupCount && !aborted; g++) {
      ...
      for (const subtask of group) {
        ...
        // Check for parent cancellation between sub-tasks
        if (parentSession.abortSignal?.aborted) {
          aborted = true;
          break;
        }
        ...
        const result = await parentSession.fork(subtask, childManifest, {
          maxToolCalls:
            subtask.estimatedBudget > 0
              ? Math.min(192, Math.max(24, Math.ceil(subtask.estimatedBudget / 128)))
              : undefined,
        });
  ...
  private static async runParallelMode(
    ...
    for (let g = 0; g < groupCount && !aborted; g++) {
      ...
      for (let batchStart = 0; batchStart < group.length && !aborted; batchStart += batchSize) {
        ...
        // Check for parent cancellation between batches
        if (parentSession.abortSignal?.aborted) {
          aborted = true;
          break;
        }
