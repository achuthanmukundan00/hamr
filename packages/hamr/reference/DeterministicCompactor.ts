...
export class DeterministicCompactor {
  ...
  constructor(options: DeterministicCompactorOptions = {}) {
    ...
    if (options.disabled) {
      ...
      // Ordered by effectiveness (most token reduction first) so budget-aware
      // compaction stops applying techniques once the target is reached.
      this.techniques = [
  ...
  compact(messages: AgentMessage[]): { messages: AgentMessage[]; stats: CompactionStats } {
    ...
    if (this.tokenBudget !== undefined && originalTokens <= this.tokenBudget) {
      return {
        messages,
        stats: {
          originalTokens,
          savedTokens: 0,
          afterTokens: originalTokens,
          techniques: [],
          breakdown: {},
        },
      };
    ...
    // For each technique, process all messages then check if the budget is met
    // before proceeding to the next (more expensive) technique.
    for (const technique of this.techniques) {
      let techniqueSaved = 0;
      ...
      for (const msg of messages) {
        ...
        if (result.changed) {
          ...
          // Only apply if the technique actually reduces token count.
          // Some techniques can inflate content (e.g. replacing short blank
          // lines with a longer annotation), which defeats the purpose.
          if (saved > 0) {
            msg.content = result.text;
            totalSaved += saved;
            techniqueSaved += saved;
          }
        }
      }
      ...
      if (techniqueSaved > 0) {
        ...
        breakdown[tname] = (breakdown[tname] ?? 0) + techniqueSaved;
        techniques.push(tname);

        // Check budget after each technique completes across all messages.
        if (this.tokenBudget !== undefined) {
          const currentTokens = originalTokens - totalSaved;
          if (currentTokens <= this.tokenBudget) {
            break; // Budget met — stop applying further techniques.
          }
        }
      }
    }
