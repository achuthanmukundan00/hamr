...
describe('streaming parser — reasoning_content edge cases', () => {
  ...
  test('normalizes cumulative reasoning_content partials into true deltas', async () => {
    srv = await createMockServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'text/event-stream' });
      for (const reasoning_content of ['think', 'think more', 'think more now']) {
        res.write(
          `data: ${JSON.stringify({
    ...
    expect(deltas.filter((d) => d.reasoningContent).map((d) => d.reasoningContent)).toEqual(['think', ' more', ' now']);
    expect(resp.reasoningContent).toBe('think more now');
  });

  test('normalizes cumulative content partials into true deltas', async () => {
    srv = await createMockServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'text/event-stream' });
    ...
    expect(deltas.filter((d) => d.content).map((d) => d.content)).toEqual(['Hel', 'lo', ' world']);
