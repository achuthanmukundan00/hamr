...
import type { Database } from '../store/sqlite-loader.js';
...
export class HolographicMemory {
  ...
  private recentEntriesStmt: Database.Statement | null = null;
  ...
  constructor(db: Database.Database | null) {
    ...
    if (db) {
      try {
        ...
        this.recentEntriesStmt = db.prepare(
        ...
        this.searchSnippetStmt = db.prepare(
        ...
        this.handoffCountsStmt = db.prepare(
  ...
  store(entry: MemoryEntry): void {
    ...
    try {
      ...
      if (!this._storeErrorWarned && this.storeErrorCount >= 3) {
        ...
        console.error(
          `[hamr] HolographicMemory: ${this.storeErrorCount} store() failures. FTS5 may be unavailable or corrupt.`,
        );
      }
    }
  }
  ...
  search(query: string, limit: number = 10): MemorySearchResult[] {
    ...
    try {
      const rows = this.searchStmt.all({ query: safeQuery, limit }) as Array<{
        turn_id: number;
        session_id: string;
        role: string;
  ...
  searchWithSnippets(query: string, limit: number = 5): Array<MemorySearchResult & { snippet: string }> {
    ...
    try {
      const rows = this.searchSnippetStmt.all({ query: safeQuery, limit }) as Array<{
        turn_id: number;
        session_id: string;
        role: string;
        tool_name: string | null;
  ...
  getSuggestedSearchTerms(): string[] {
    if (!this.recentEntriesStmt) return [];

    // Return cached result if memory hasn't changed
    if (this._suggestedTermsCache) return this._suggestedTermsCache;

    try {
      // Single query: fetch last 30 entries, derive tools/domains/words in JS
      const recent = this.recentEntriesStmt.all({ limit: 30 }) as Array<{
        tool_name: string | null;
        domain_tags: string | null;
        role: string;
        content: string;
  ...
  private _computeSuggestedTerms(
    ...
    for (const entry of entries) {
      // Collect tool names
      if (entry.tool_name) toolNames.add(entry.tool_name);

      // Collect domain tags
      if (entry.domain_tags) {
        for (const tag of entry.domain_tags.split(',')) {
          const trimmed = tag.trim();
          if (trimmed) domainTagSet.add(trimmed);
        }
      }
    ...
    for (const t of toolNames) terms.add(t);
    for (const d of domainTagSet) terms.add(d);
  ...
  buildMemoryIndex(): string | null {
    if (!this.statsStmt || !this.recentEntriesStmt) return null;

    try {
      // Query 1: stats — count entries and turns
      const stats = this.statsStmt.get() as { entries: number; turns: number } | undefined;
        ...
        if (entry.role !== 'user' && errorLines.length < 3) {
          ...
          if (lower.includes('error') || lower.includes('fail')) {
            ...
            for (const line of lines) {
              ...
              if (
                (lineLower.includes('error') || lineLower.includes('fail')) &&
                line.length > 15 &&
                line.length < 200
              ) {
