...
import { loadHamrConfig } from './load-config.js';
import type { EffectiveHamrConfig, ResolvedProviderConfig, ThinkingLevel } from './schema.js';
...
export interface AgentBudgetConfig {
  ...
  /** Injected into the system prompt once at conversation start. Token-efficient behavioral mandate. */
  mandate?: string;
}
...
): import('./types.js').NormalizedProviderConfig {
...
export interface ProjectConfig {
  ...
  maxTotalReadResultTokensPerTurn?: number;
  max_total_read_result_tokens_per_turn?: number;
  /** Injected into the system prompt once at conversation start. */
  mandate?: string;
...
export function applyEffectiveHamrConfigToProjectConfig(
  current: ProjectConfig,
  effective: EffectiveHamrConfig,
): ProjectConfig {
  ...
  // Model-specific context window takes priority over stale probe/config values.
  // Fallback order: model default → provider default → user config → global default.
  const contextWindow =
...
export function generateDefaultConfig(): string {
  return [
    ...
    'max_tool_calls = 192',
    '# mandate = "Use short sentences. Be concise."  # injected once at conversation start',
    '',
    '[subagents]',
    'enabled = false',
    'mode = "sequential"',
...
function configFromParsedToml(parsed: Record<string, unknown>): ProjectConfig {
  ...
  if (agent?.mandate !== undefined) config.mandate = agent.mandate;
  if (parsed.mandate !== undefined) config.mandate = parsed.mandate as string;
...
function applyEnvOverrides(config: ProjectConfig, errors: ValidationError[]): ProjectConfig {
  ...
  const contextBudgetTokens = readEnvPositiveInteger('HAMR_CONTEXT_BUDGET_TOKENS', errors);
  const maxToolCalls = readEnvPositiveInteger('HAMR_MAX_TOOL_CALLS', errors);
...
export function loadProjectConfig(baseDir?: string): LoadProjectConfigResult {
  ...
  const errors: ValidationError[] = [];
  const userConfigPath = process.env.HOME ? join(process.env.HOME, '.config', 'hamr', 'config.toml') : null;
  let userConfig: ProjectConfig = {};
  let hasExtendedSettings = false;
  // Capture raw TOML content to avoid re-reading files when loadHamrConfig is called.
  ...
  const hasExplicitContextWindow =
    ...
    process.env.HAMR_CONTEXT_BUDGET_TOKENS !== undefined;
  if (hasExtendedSettings) {
    const effectiveSettings = loadHamrConfig(baseDir, { globalRaw, localRaw });
    const effectiveProjectConfig = applyEffectiveHamrConfigToProjectConfig({}, effectiveSettings);
    // When multi-provider settings are active, the project .hamr.toml's
  ...
  if (hasExtendedSettings) {
    // When using new multi-provider format, provider is already resolved by loadHamrConfig.
  ...
  return {
    ...
    path,
    source:
      path === null ? 'default' : basename(path) === '.hamr.toml' && dirname(path) !== baseDir ? 'file' : 'explicit',
  };
}
