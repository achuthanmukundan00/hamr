...
Hamr skills extend the agent's capabilities by injecting skill instructions
into the agent's system context before each model step.

Skills come from two sources:

1. **Config-based skills** (`.hamr.toml` `[skills]`): explicit path entries to
...
2. **Auto-discovered skills**: Hamr scans `~/.hamr/skills/` (global) and
   `.hamr/skills/` (project-local) for subdirectories containing `SKILL.md`
...
Hamr scans two directories for `SKILL.md` files:

- `~/.hamr/skills/<skill-name>/SKILL.md` — global skills (user-installed)
- `.hamr/skills/<skill-name>/SKILL.md` — project-specific skills
...
`[skills]` config section in `.hamr.toml`:
...
```bash
hamr run --task "help me draft a resume" --no-skills
hamr chat --no-skills
```

## Config Path Skill Format (detailed)

For explicit config-based skill entries, Hamr resolves paths to files. Hamr resolves the path to
...
If the resolved path is a directory, Hamr appends `/SKILL.md` automatically.
...
```bash
hamr inspect --skills           # List all discovered skills
hamr inspect --skill <name>     # Show full instructions for a skill
```

## Diagnostics

At startup, Hamr resolves each enabled skill path and reports:
