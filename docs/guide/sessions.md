...
Hamr persists sessions so you can resume previous work.

## Session Storage

Sessions are stored under `~/.local/share/hamr/sessions/`:

```
~/.local/share/hamr/sessions/
...
| Workspace path             | Assistant stream deltas          |
| Git branch                 | Tool calls and results           |
| Active provider/model      | State snapshots                  |
| Message/event counts       | Summaries                        |
| Session status             |                                  |
...
> 15 min ago     12 min ago        4 active     dev/tui     qwen-local  Fix Hamr TUI input corruption bugs...
  31 min ago     28 min ago        2 completed  dev/tui     qwen-local  Please look at specs/hamr-ai-core...
...
3. User messages and assistant text/thinking streams are restored behind the stable system/skill prefix
4. Tool calls/results are replayed only as valid paired tool turns; state snapshots stay log-only
...
- Assistant stream deltas are compacted into replayable message blocks during resume
