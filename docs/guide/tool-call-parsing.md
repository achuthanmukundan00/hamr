...
Hamr also normalizes messy streaming output from local gateways. Some servers send
cumulative partials instead of true deltas; Hamr converts them to append-only
text before updating the TUI or session log. Thinking/reasoning text is displayed
as part of the live assistant message, then kept out of future prompts unless the
provider explicitly requires reasoning replay.
