...
Hamr treats context as a budget and file edits as explicit local operations.

## Context Management

Hamr manages context at runtime with deterministic controls:

### Working Context Orientation

After each model turn that involves file reads, Hamr injects a compact "WORKING CONTEXT" block
...
When estimated context exceeds ~60% of the effective limit, Hamr automatically compacts
...
- Edit safety is preserved: if exact source text was truncated, Hamr
  requires a reread before accepting an edit

### Model Message Assembly

Before every model call, Hamr builds the message array through a proactive
...
```sh
hamr inspect --budget     # Show budget configuration
hamr inspect --ledger     # Show working context state from last chat session
hamr inspect --context    # Show expanded context state (JSON)
```
...
Control how aggressively Hamr compacts conversation context via `[agent] context_strategy`:
...
long-horizon exploration is not cut off prematurely. Large-context models should
keep raw context unless they are genuinely near the effective limit. Small local
models get the safety rails: capped reads, deterministic compaction, strict
pair-preserving tool history, and handoff/FTS5 recovery when needed. Re-reading a
range that was already read this turn is always served from the read cache and
does not consume budget. When the per-turn cap is reached, further new reads are
refused with a recoverable policy error that does not terminate the turn; the
agent is steered toward acting on what it has (edit/write/bash) or recalling
earlier reads via `search_memory`. Set either key explicitly to disable scaling
for that cap.
...
Hamr rejects unsafe file paths and generated outputs. It is designed to avoid reading or editing:
...
Validated replacement edits emit a patch preview before Hamr writes the file. In non-interactive
`hamr run --task` sessions, previewed replacement edits are rejected by default; pass `--yes` to accept them for that
...
If a read result was truncated for context safety, Hamr will not accept that truncated session as proof for exact replacement edits.
...
`hamr run --task` also reports verification state when a command is configured.
...
Hamr exposes git status and diff through bounded read-only tools. It does not reset or clean the repository. Review changes with:
...
For run/task safety artifacts, Hamr records:

- lazy checkpoints under `.hamr/checkpoints/` once the first allowed mutation is about to run
- bounded run logs under `.hamr/runs/`
- last Hamr-owned edit metadata at `.hamr/last-edit.json` for `/undo-last-edit`
