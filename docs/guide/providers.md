...
Hamr resolves the variable from `process.env` at runtime. If the env var is unset or empty, the header is omitted entirely.
...
# Test Groq
HAMR_LIVE_PROVIDER=groq bun run smoke:provider
...
Each smoke test sends a tiny prompt (`"Reply with exactly: hamr-ok"`)
