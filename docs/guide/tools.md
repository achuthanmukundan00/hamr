...
Hamr provides a set of built-in tools for local model inference. External consumers can also register custom tools programmatically.
...
When using Hamr as an SDK, you can register custom tools that participate in budget enforcement, lifecycle events, and the session context exactly like built-in tools.

```ts
import { Session } from 'hamr/session';
import type { ToolResult } from 'hamr/tools';
