...
Hamr also accepts `core_visual_profile` and legacy `[provider]`-scoped placement for compatibility.
...
Changes persist to the local `.hamr.toml` if one exists, otherwise to the global config.
