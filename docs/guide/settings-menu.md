...
The interactive settings menu is the primary way to configure Hamr at runtime.
...
Lists auto-discovered skills from `~/.hamr/skills/` and `.hamr/skills/`, plus
config-based skills from `.hamr.toml` (e.g. `~/.agents/skills/coderabbit-review`):
...
- If a local `.hamr.toml` exists, changes go there
- Otherwise, changes go to `~/.config/hamr/config.toml`
- If neither exists, a new `.hamr.toml` is created in the current repo

## Visual Style

The settings menu follows Hamr's industrial design:
