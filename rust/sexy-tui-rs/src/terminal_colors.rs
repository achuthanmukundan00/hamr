...
pub fn is_osc11_background_color_response(data: &str) -> bool {
    // Strict pattern: \x1b]11;<value>\x07 or \x1b]11;<value>\x1b\\
    if !data.starts_with("\x1b]11;") {
        return false;
    }
    let rest = &data[5..]; // skip "\x1b]11;"
    if rest.ends_with('\x07') {
        !rest[..rest.len() - 1].contains('\x07') && !rest[..rest.len() - 1].contains("\x1b\\")
    } else if rest.ends_with("\x1b\\") {
        !rest[..rest.len() - 2].contains('\x07') && !rest[..rest.len() - 2].contains("\x1b\\")
    } else {
        false
    }
}
...
/// Terminal color scheme report parsed from CSI ?997;N n.
pub type TerminalColorScheme = String;

/// Parse terminal color scheme reports: `ESC[?997;1n` = dark, `ESC[?997;2n` = light.
pub fn parse_terminal_color_scheme_report(data: &str) -> Option<TerminalColorScheme> {
    match data {
        "\x1b[?997;1n" => Some("dark".to_string()),
        "\x1b[?997;2n" => Some("light".to_string()),
        _ => None,
    }
}
