...
/// Check if a line contains a terminal inline image escape sequence.
...
pub fn encode_kitty(
    ...
    let cols = if let Some(max_w) = opts.max_width_cells {
        ((dims.width_px as f64 / cell_dims.width_px as f64).ceil() as u32).min(max_w)
    } else {
...
pub fn calculate_image_rows(
    ...
    let adjusted_height =
        (dims.width_px as f64 / cell_dims.width_px as f64 / aspect * cell_aspect).ceil() as u32;
