...
pub fn is_native_modifier_pressed(_key: ModifierKey) -> bool {
    // Crossterm's KeyEvent provides modifier state per-event, which covers
    // the primary use case (shift+enter detection, etc.). CGEventSource
    // polling is not needed since the TUI processes key events directly.
    false
}
