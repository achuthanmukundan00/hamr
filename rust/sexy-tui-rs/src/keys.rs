...
pub fn decode_kitty_printable(data: &str) -> Option<char> {
    ...
    // Also: ESC [ <codepoint>:<shifted>:<base> ; <modifier> u
    let body = data.strip_prefix("\x1b[")?;
    let body = body.strip_suffix('u')?;
    // Get the codepoint part (before semicolon, strip colon-separated suffixes)
    let codepoint_str = body.split(';').next()?;
    let codepoint_str = codepoint_str.split(':').next()?;
...
/// Alternate keys: ESC [ <codepoint>:<shifted>:<base> ; <modifier> u
fn parse_kitty_sequence(data: &str) -> Option<(u32, u8)> {
    let body = data.strip_prefix("\x1b[")?;
    let body = body.strip_suffix('u')?;

    // Split by ';' to get codepoint-info and modifier
    let mut parts = body.split(';');
    let cp_info = parts.next()?;
    let modifier: u8 = parts.next().unwrap_or("1").parse().ok()?;

    // cp_info may be "<codepoint>" or "<codepoint>:<shifted>:<base>"
    // Get the primary codepoint (first colon-separated segment)
    let codepoint: u32 = cp_info.split(':').next()?.parse().ok()?;
    Some((codepoint, modifier - 1)) // Kitty mods are 1-indexed
}

/// Match a Kitty keyboard sequence using either primary codepoint or base layout codepoint.
/// Base codepoint is only considered for non-Latin primary codepoints (Cyrillic, etc.).
fn matches_kitty_sequence(data: &str, expected_codepoint: u32, expected_modifier: u8) -> bool {
    if let Some((codepoint, modifier)) = parse_kitty_sequence(data) {
        ...
        // For shifted keys: try the non-shifted codepoint too
        if expected_modifier & modifiers::SHIFT != 0 {
            if let Some(shifted) = shift_codepoint(expected_codepoint) {
                if codepoint == shifted {
                    return true;
                }
            }
        }
        // Base layout codepoint: only use for non-ASCII primary codepoints (Cyrillic, etc.)
        if !is_ascii_codepoint(codepoint) {
            if let Some(base_cp) = parse_kitty_base_codepoint(data) {
                if base_cp == expected_codepoint {
                    return true;
                }
            }
        }
...
/// Check if a codepoint is in the ASCII range (standard US keyboard).
fn is_ascii_codepoint(cp: u32) -> bool {
    cp <= 0x7F
}

/// Given a lowercase letter codepoint, return the uppercase version.
fn shift_codepoint(cp: u32) -> Option<u32> {
...
/// Extract the base layout codepoint from Kitty alternate key format.
/// Format: ESC [ <cp>:<shifted>:<base> ; <mod> u  or  ESC [ <cp>::<base> ; <mod> u
fn parse_kitty_base_codepoint(data: &str) -> Option<u32> {
    let body = data.strip_prefix("\x1b[")?;
    let body = body.strip_suffix('u')?;
    let cp_info = body.split(';').next()?;
    let colon_parts: Vec<&str> = cp_info.split(':').collect();
    // Format: cp ; mod  → no base
    // Format: cp : shifted : base ; mod → base is index 2
    // Format: cp : : base ; mod → base is index 2
    if colon_parts.len() >= 3 {
        colon_parts[2].parse().ok()
    } else {
        None
    }
}
...
/// Parse a modifyOtherKeys sequence: CSI <prefix> ; <modifier> ; <codepoint> ~
/// The prefix is usually 27 for modifyOtherKeys format.
fn parse_modify_other_keys_sequence(data: &str) -> Option<(u32, u8)> {
    let body = data.strip_prefix("\x1b[")?;
    let body = body.strip_suffix('~')?;
    let mut parts = body.split(';');
    let _prefix: u32 = parts.next()?.parse().ok()?;
    let modifier: u8 = parts.next().unwrap_or("1").parse().ok()?;
...
fn matches_modify_other_keys(data: &str, expected_codepoint: u32, expected_modifier: u8) -> bool {
    if let Some((codepoint, modifier)) = parse_modify_other_keys_sequence(data) {
        ...
        // For shifted keys: try the shifted codepoint
        if expected_modifier & modifiers::SHIFT != 0 {
            if let Some(shifted) = shift_codepoint(expected_codepoint) {
                if codepoint == shifted {
                    return true;
...
pub fn matches_key(data: &str, key_id: &str) -> bool {
    ...
    match parsed.key.as_str() {
        ...
        "enter" | "return" => {
            if modifier == 0 && (data == "\r" || data == "\n") {
                return true;
            }
            if modifier == modifiers::ALT && (data == "\x1b\r" || data == "\x1b\n") {
                return true;
            }
        ...
        key_str => {
            ...
            // Legacy Ctrl+symbol mappings (ASCII control chars)
            if modifier == modifiers::CTRL && key_str.len() == 1 {
                let ch = key_str.chars().next().unwrap();
                let legacy_match = match ch {
                    '\\' => data == "\x1c",
                    ']' => data == "\x1d",
                    '_' | '-' => data == "\x1f",
                    _ => false,
            ...
            if let Some(ch) = key_str.chars().next() {
                if key_str.len() == 1 {
                    return matches_kitty_sequence(data, ch as u32, modifier)
                        || matches_modify_other_keys(data, ch as u32, modifier);
                }
            }
            false
        }
