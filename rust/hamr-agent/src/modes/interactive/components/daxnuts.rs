//! POWERED BY DAXNUTS — Easter egg for OpenCode + Kimi K2.5
//!
//! Port of `packages/coding-agent/src/modes/interactive/components/daxnuts.ts`.
//!
//! A heartfelt tribute to dax (@thdxr) for providing free Kimi K2.5 access via OpenCode.

use crate::modes::interactive::components::tui_shim::Component;
use crate::modes::interactive::theme::theme::theme;

/// 32×32 RGB image of dax, hex encoded (3 bytes per pixel).
const DAX_HEX: &str = "\
";

const WIDTH: usize = 32;
const HEIGHT: usize = 32;

/// Parse the hex image data into a 2D array of [r, g, b] values.
fn parse_image() -> Vec<Vec<[u8; 3]>> {
    let mut pixels = Vec::with_capacity(HEIGHT);
    let bytes = DAX_HEX.as_bytes();
    for y in 0..HEIGHT {
        let mut row = Vec::with_capacity(WIDTH);
        for x in 0..WIDTH {
            let idx = (y * WIDTH + x) * 6;
            let r = hex_byte(bytes[idx], bytes[idx + 1]);
            let g = hex_byte(bytes[idx + 2], bytes[idx + 3]);
            let b = hex_byte(bytes[idx + 4], bytes[idx + 5]);
            row.push([r, g, b]);
        }
        pixels.push(row);
    }
    pixels
}

fn hex_byte(hi: u8, lo: u8) -> u8 {
    (hex_nibble(hi) << 4) | hex_nibble(lo)
}

fn hex_nibble(b: u8) -> u8 {
    match b {
        b'0'..=b'9' => b - b'0',
        b'a'..=b'f' => b - b'a' + 10,
        b'A'..=b'F' => b - b'A' + 10,
        _ => 0,
    }
}

/// Build an ANSI escape sequence for foreground RGB color.
fn rgb_fg(r: u8, g: u8, b: u8) -> String {
    format!("\x1b[38;2;{};{};{}m", r, g, b)
}

/// Build an ANSI escape sequence for background RGB color.
fn rgb_bg(r: u8, g: u8, b: u8) -> String {
    format!("\x1b[48;2;{};{};{}m", r, g, b)
}

const RESET: &str = "\x1b[0m";

/// Build the half-block rendered image lines.
fn build_image() -> Vec<String> {
    let pixels = parse_image();
    let mut lines = Vec::new();

    for row in (0..HEIGHT).step_by(2) {
        let mut line = String::new();
        for x in 0..WIDTH {
            let top = pixels[row][x];
            let bottom = if row + 1 < HEIGHT {
                pixels[row + 1][x]
            } else {
                top
            };
            // Use half-block char: ▄ with bg=top pixel, fg=bottom pixel
            line.push_str(&rgb_fg(bottom[0], bottom[1], bottom[2]));
            line.push_str(&rgb_bg(top[0], top[1], top[2]));
            line.push('▄');
        }
        line.push_str(RESET);
        lines.push(line);
    }
    lines
}

/// Animated daxnuts easter egg component with scanline reveal effect.
pub struct DaxnutsComponent {
    image: Vec<String>,
    tick: usize,
    max_ticks: usize, // ~2 seconds at 80ms
    cached_lines: Vec<String>,
    cached_width: u16,
    cached_tick: usize,
}

impl DaxnutsComponent {
    pub fn new() -> Self {
        Self {
            image: build_image(),
            tick: 0,
            max_ticks: 25,
            cached_lines: Vec::new(),
            cached_width: 0,
            cached_tick: usize::MAX,
        }
    }

    /// Advance the animation by one tick.
    pub fn tick(&mut self) {
        if self.tick < self.max_ticks {
            self.tick += 1;
            self.cached_width = 0; // force re-render
        }
    }

    /// Whether the animation is done.
    pub fn is_done(&self) -> bool {
        self.tick >= self.max_ticks
    }
}

impl Component for DaxnutsComponent {
    fn render(&self, width: u16) -> Vec<String> {
        if width == self.cached_width && self.cached_tick == self.tick {
            return self.cached_lines.clone();
        }
        // We need to bypass borrow checker — render needs &mut self but Component::render is &self.
        // In the real TUI integration this uses interior mutability.
        // For now, we compute fresh each time.
        let t = theme();
        let mut lines: Vec<String> = Vec::new();

        let center = |s: &str| -> String {
            let visible = crate::modes::interactive::components::tui_shim::visible_width(s);
            let left = ((width as i32 - visible as i32).max(0) / 2) as usize;
            format!("{}{}", " ".repeat(left), s)
        };

        lines.push(String::new());

        // Scanline reveal effect
        let revealed_rows = (self.image.len() as f64 * (self.tick as f64 / self.max_ticks as f64))
            .min(self.image.len() as f64 + 3.0) as usize;

        for i in 0..self.image.len() {
            if i < revealed_rows {
                lines.push(center(&self.image[i]));
            } else if i == revealed_rows {
                let scanline = "▓".repeat(WIDTH);
                lines.push(center(&format!(
                    "{}{}{}",
                    rgb_fg(100, 200, 255),
                    scanline,
                    RESET
                )));
            } else {
                lines.push(center(&" ".repeat(WIDTH)));
            }
        }

        lines.push(String::new());

        // Fade in text after image is revealed
        let text_phase = if self.tick > (self.max_ticks as f64 * 0.6) as usize {
            self.tick - (self.max_ticks as f64 * 0.6) as usize
        } else {
            0
        };

        if text_phase > 0 || self.tick >= self.max_ticks {
            lines.push(center(&t.fg("accent", "Free Kimi K2.5 via OpenCode Zen")));
            lines.push(center(&t.fg("success", "\"Powered by daxnuts\"")));
            lines.push(center(&t.fg("muted", "— @thdxr")));
        } else {
            lines.push(String::new());
            lines.push(String::new());
            lines.push(String::new());
        }

        lines.push(String::new());
        if text_phase > 2 || self.tick >= self.max_ticks {
            lines.push(center(&t.fg("dim", "Try OpenCode")));
            lines.push(center(
                &t.fg("mdLink", "https://mistral.ai/news/mistral-vibe-2-0"),
            ));
        } else {
            lines.push(String::new());
            lines.push(String::new());
        }
        lines.push(String::new());

        lines
    }

    fn invalidate(&mut self) {
        self.cached_width = 0;
        self.cached_tick = usize::MAX;
    }
}

impl Drop for DaxnutsComponent {
    fn drop(&mut self) {
        // Animation cleanup — in TS this calls stopAnimation/clearInterval
    }
}
